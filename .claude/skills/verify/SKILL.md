...
| **게이트를 세우거나 고쳤으면** | 청크 | `bash scripts/gate-probe.sh <이름>`(`Q225`) — 트리가 깨끗해야 돈다. 새 게이트면 `scripts/probes/<이름>.patch` 를 먼저 만든다 | 「통과 — 부순 채로 빨강이었고 트리는 전과 같다」. `verify.sh` 는 안 부른다 — 탐침마다 빌드가 붙는다 |
