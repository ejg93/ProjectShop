...
| 시드·데모 데이터를 건드렸으면 | `./gradlew bootRun --args='--spring.profiles.active=local'` | `db/seed/` 가 같이 적용된다. 계정 16·셀러 2, 비밀번호는 `test@test.local` 만 `test-account-1234` 고 나머지는 `demo-password-1234` 다. 그중 아홉이 로그인 화면에 공개된다(`Q130`·`Q131`). **`local` 없이 뜨면 시드가 안 들어간다** |
