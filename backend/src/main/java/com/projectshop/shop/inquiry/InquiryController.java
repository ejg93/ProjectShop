...
public class InquiryController {
     ...
     *
     * <p><b>전체 목록만 관객이 경로에 없다.</b> 같은 컨트롤러에 `/api/me/inquiries`·`/api/seller/inquiries`
     * 가 있으므로 「관객을 경로에 안 박는다」는 이 저장소의 규칙이 아니다 — 그쪽 둘은 **볼 범위가 경로로
     * 갈리고** 여기는 판정이 가른다(`D22` 「경로가 관객이어도 패키지는 자원이다」와 같은 결).
     * 처음 적을 때 이 자리를 규칙처럼 써 뒀고 마무리 15차 리뷰가 짚었다.
