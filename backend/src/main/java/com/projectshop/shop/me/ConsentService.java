...
public class ConsentService {
    ...
    public List<ConsentView> list(long userId) {
        ...
        return jdbc.sql("""
                        select ci.code, ci.title, ci.is_required, p.code as depends_on,
                          ...
                          from (
                              select distinct on (code) id, code, title, is_required, depends_on_id
                                from consent_item
                               where effective_at <= now()
                               order by code, effective_at desc, version desc
                          ) ci
                          left join consent_item p on p.id = ci.depends_on_id
    ...
    public void revoke(long userId, String code, String actorIp) {
        ...
        for (Item dependent : findDependents(item.id())) {
    ...
    private Item findItem(String code) {
        return jdbc.sql("""
                        select ci.id, ci.code, ci.is_required, p.code as depends_on
    ...
    private List<Item> findDependents(long itemId) {
        return jdbc.sql("""
                        select id, code, is_required, null as depends_on
                          from consent_item
    ...
    private record Item(long id, String code, boolean required, String dependsOnCode) {
    }
}
