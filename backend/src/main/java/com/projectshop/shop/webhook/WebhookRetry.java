 ...
 * 셀러가 화면에서 손으로 다시 보낸다(`Q175`). <b>여덟 번이면 첫 실패부터 약 64분</b>(30초 × (2⁷ − 1) = 3,810초)에 소진된다 —
 * 가장 긴 간격이 32분이라 <b>상한 1시간은 지금 수로는 안 걸린다</b>. 상한은 횟수를 늘리는 날의 방어다.
 * 받는 쪽이 한 시간 넘게 죽어 있으면 멈추고, 셀러가 받는 쪽을 고친 뒤 다시 보낸다(마무리 47차가 옛 「약 네 시간」을 고쳤다).
 */
