...
public class PermissionEvaluator {
     ...
     */
    record Rule(String roleCode, Long grantSellerId, String scope, String effect) {
    ...
    /**
     * 판정 결과. 허용 여부만이 아니라 어느 규칙이 이겼는지를 같이 담는다.
     * 왜 막혔는지 모르면 권한 문제는 재현이 안 된다.
     */
    public record Decision(boolean allowed, String reason) {

        static Decision allow(Rule rule) {
            return new Decision(true, "허용 — " + rule);
        }

        static Decision denyBy(Rule rule) {
            return new Decision(false, "거부 — " + rule);
        }

        static Decision denyBecause(String reason) {
            return new Decision(false, "거부 — " + reason);
        }
    }
    ...
    public Decision decide(long userId, String resource, String action, Target target) {
        ...
        Set<Long> memberOf = null;

        for (Rule rule : rules) {
            if (rule.isDeny()) {
                if (memberOf == null) {
                    memberOf = loadSellerMemberships(userId);
                }
                if (covers(rule, userId, memberOf, target)) {
