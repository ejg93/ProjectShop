...
class TriggerCoverageTest extends PostgresTestBase {
    ...
    private static final Map<String, String> COVERED = Map.ofEntries(
            ...
            Map.entry("role_permission.role_permission_webhook_manage_owner_only",
                    "웹훅 manage 허용을 대표의 셀러 범위 밖으로 못 넣게 막는다(`Q211`, `V117`). `WebhookEndpointServiceTest` 가 문구까지 단언한다"),
            ...
            Map.entry("return_request.return_requires_rejection_reason",
                    "반품 거절에는 사유가 있어야 하고, 훼손 거절이면 검수 소견 글도 있어야 한다(`Q212`, `V115` 가 다시 만들었다). `ReturnDecisionTest` 가 둘 다 문구까지 단언한다"),
