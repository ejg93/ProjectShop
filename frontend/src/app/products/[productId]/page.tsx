 ...
 *
 * <p><b>서버가 준 값을 그대로 안 쓴다</b>(`D20` 「서버 문구는 `message` 만 그린다」).
 * 저장값은 열거값이라 화면 문구가 아니다.
 *
 * <p><b>「제한됩니다」로 단정하지 않는다</b>(사용자 선택). 상품에 붙은 것은
 * 「이 사유에 해당할 수 있다」는 표시고, <b>제한이 서려면 사유마다 다른 조건이 찬다</b>
 * (`D2` R4, `WithdrawalRestrictionReason`). 조건을 안 적고 단정하면 아직 제한되지 않은
 * 상품을 제한된 것처럼 말하는 것이라, <b>과장 고지 자체가 제21조 쪽 위험</b>이다.
 *
 * <p>셋이 조건이 다르다 — 포장 훼손은 <b>물건이 돌아와야 아는 사실</b>이라 접수를 막지도 않고,
 * 디지털 콘텐츠는 <b>제공 개시</b>가, 주문 제작은 <b>주문서의 별도 동의</b>(`Q6`)가 요건이다.
 */
const WITHDRAWAL_REASON_TEXT: Record<WithdrawalReason, string> = {
  COPYABLE_MEDIA: "포장을 훼손하시면 청약철회가 제한될 수 있습니다.",
  DIGITAL_CONTENT: "제공이 시작되면 청약철회가 제한됩니다.",
  MADE_TO_ORDER:
    "주문을 받고 만드는 상품입니다. 주문서에서 동의하시면 청약철회가 제한됩니다.",
};
