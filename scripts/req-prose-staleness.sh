...
# **넷**(제약명·클래스명·파일명·마이그레이션 번호)뿐이고 요건표 자신이 「경로·청크 번호·산문은 안 잰다」고 적어 뒀다.
...
# 양쪽에서 빠져 있었다. **추출에 `Q` 로 시작하는 행을 잡는 분기가 아예 없었고** 패턴도 숫자로
# 시작하는 것만 봤다. **빠진 것은 게이트가 스스로 못 알려 준다** — 그래서 `--selftest` 가 여섯 모양을 잰다.
#
# **`Q152` 가 적은 원인은 틀렸었다**(PR #65 리뷰가 잡았다). 「`Q` 행은 상태 칸 없이 마지막 칸이
# 커밋 해시」라고 그 행 하나만 보고 일반화했다. 실물은 **닫힘 신호가 둘**이다 — 마지막 칸이
# `완료`(59) 이거나 제목에 취소선(144)이고 합집합 148이 닫힌 것 전부다. 해시로 끝나는 행도
# 15 있지만(`Q1`~`Q15`) **그 열다섯은 전부 취소선을 같이 든다.** 고친 코드는 돌았지만 **왜 돌았는지가 거짓**이었고,
# 그 거짓 전제로 만든 판정(`desc ~ /완료/`)이 **미착수 `Q61` 을 닫힌 것으로 셌다**(설명에 「완료 뒤 → 재생」이 있다).
set -uo pipefail
cd "$(dirname "$0")/.."
...
# 분할표에서 닫힌 청크 번호. **행 모양은 같다** — 숫자든 `Q` 든 마지막에서 두 번째 칸이
# 상태(`완료`)나 선행이다. 다만 **`Q` 는 닫혔어도 그 칸에 선행이 남는 것이 많아서**
# 제목의 취소선을 같이 본다(실측: 153개 중 취소선 144·마지막 칸 `완료` 59, 둘을 합쳐야 148).
# **설명 칸의 「완료」로는 안 센다** — 미착수 `Q61` 의 설명에 「완료 뒤 → 재생」이 있다.
collect_done() {
  awk -F'|' '
    /^\| *[0-9]/ {
      id=$2; gsub(/[ `~*]/, "", id);
...
# **여섯 모양을 따로 잰다**(`Q152`). 실물이 통과하는 것만으로는 정규식이 성립했다는 증거가 안 된다.
if [ "${1:-}" = "--selftest" ]; then
  tmp=$(mktemp -d)
  trap 'rm -rf "$tmp"' EXIT
  cat > "$tmp/plan.md" <<'FIX'
| 5j | ~~동의 고지 항목~~ | **완료.** | 완료 |
| Q13 | ~~정보주체 권리~~ | **완료.** | Q12 |
| Q14 | 취소선 없이 닫힌 것 | **완료.** | 완료 |
| Q61 | 아직 안 한 것 | 설명에 「완료 뒤 → 재생」이 있다 | 없음 |
| 99z | 아직 안 한 것 | 설명 | |
FIX
  ...
  check "취소선 없이 닫힌 Q 청크도 걸려야 한다"              '청크 `Q14` 가 채운다.' 1
  check "현재형 서술은 안 걸려야 한다"                      '`RefundSweeper`(`5j`)가 요청을 만든다.' 0
  check "미착수 청크를 미래형으로 — 안 걸려야 한다"          '청크 `99z` 가 채운다.' 0
  check "설명에 「완료」가 있어도 미착수면 안 걸려야 한다"    '청크 `Q61` 이 채운다.' 0
  [ "$fail" -eq 0 ] && echo "여섯 모양 전부 통과" || echo "여섯 모양 중 실패가 있다"
  exit "$fail"
fi
