...
| 2026-08-07 | 5e 중 드러난 것 | **`JdbcClient.query().singleRow()` 는 `timestamptz` 를 `java.sql.Timestamp` 로 준다.** `OffsetDateTime` 으로 캐스팅하면 `ClassCastException` 이고, 컴파일은 통과해서 실행할 때만 드러난다. `AuditLogQuery` 는 RowMapper 를 써서 안 걸렸던 자리다. 시각 컬럼을 읽을 때는 `Map` 이 아니라 RowMapper 로 받는다 | |
## 기록 규칙
