...
- **진행중 청크**: 없음
- **다음에 할 것**: **8a → 4b-1**. 1차 점검이 정한 세로 관통 순서의 남은 둘이다.
  가입·로그인·로그아웃이 실서버에서 통째로 돈다. `8a` 가 **판정 엔진을 요청 흐름 안에서 처음 부른다.**

  **`8a` 를 짤 때**: 로그인 응답은 누구인지만 내려준다. 무엇을 할 수 있는지가 `8a` 의 몫이고,
  `UserDetails` 에 권한을 안 담아 뒀으므로 `PermissionEvaluator` 를 불러야 한다.
  인증된 사용자 id 는 `SecurityContextHolder` 의 principal(`ShopUserDetailsService.ShopUser`)에서 꺼낸다.

  **테스트에서 실제 로그인을 부르면 `@AfterEach` 로 `SecurityContextHolder.clearContext()`** 를 한다.
  안 하면 다음 테스트 클래스가 인증된 채로 시작한다 — `stack.md` 에 적힌 함정이다.

  그 뒤에 **계정 관리 축(5h·5e·5f·5g)** 이 기다린다.
