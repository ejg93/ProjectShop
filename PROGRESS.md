...
- **진행중 청크**: 없음
- **다음에 할 것**: **4b-1 감사 로그 조회**. 세로 관통의 마지막 조각이다.
  가입·로그인·권한 목록이 실서버에서 이어진다. 남은 것은 **쌓인 감사 기록을 꺼내 보는 것**이고,
  거기서 **필드 마스킹(4d)이 실제 응답에 처음 걸린다.**

  **`4b-1` 을 짤 때**: 조회 자체가 권한을 타야 한다 — 아무나 남의 감사 기록을 보면 안 된다.
  `PermissionEvaluator.decide` 를 부르는 첫 자리가 될 것이다(`8a` 는 `evaluate` 만 썼다).
  `audit_log.actor_user_id` 에는 외래키가 없다(4b) — 조인할 때 계정이 없을 수 있다.

  그 뒤에 **2차 점검**, 그리고 **계정 관리 축(5h·5e·5f·5g)** 이 기다린다.
