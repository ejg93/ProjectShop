...
- **진행중 청크**: 없음
- **다음에 할 것**: **`D20` 화면·문구 규약 → 6·7 상품 축**.
  `D22`·`D23` 규칙 준수 점검은 끝났다(`checkpoint-3-rules.md`). 상품 축은 깨끗한 기준 위에서 시작한다.
...
| 문서 | 지금 쓸 수 있나 | 왜 |
|---|---|---|
| **D20 화면·문구** | **쓸 수 있다** | 선행 `D5` 완료. `8a` 가 권한 목록을 내려줘서 "권한 없는 버튼" 을 정할 근거가 생겼다. 화면 청크 7개가 기다린다. **권한 없는 버튼은 숨기기로 사용자가 정했다** |
