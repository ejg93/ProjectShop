...
- **진행중 청크**: 없음
- **다음에 할 것**: **청크 5 → 8a → 4b-1** 순서로 간다. 1차 점검이 정한 순서다 —
  판정 엔진·감사 로그·마스킹이 전부 호출자 없이 서 있어서, 요청 하나가 그것들을 실제로 지나가게 만드는 것이 먼저다.
  셋 다 선행이 이미 만족돼 있다. 그 뒤에 6·7 상품 축으로 간다.

  **청크 5 (회원가입·로그인)**. 스키마(5-0)와 인증 기반(5-1)이 깔렸다. 남은 것은 엔드포인트다.
...
| 2026-08-07 | **1차 점검** | 완료 — `doc/notes/checkpoint-1.md`. 청크 0~5-1 을 거시로 훑었다. **결함 2개를 고쳤다**: `PLAN.md` 160행이 깨져 청크 4 행이 3d 행 끝에 붙어 있었고(표를 훑는 재개 절차가 못 본다), 이력의 커밋 해시 4칸이 비어 청크와 커밋을 대조할 수 없었다. **가장 큰 관찰: 23% 왔는데 엔드포인트가 `/api/health` 하나뿐이다** — 판정 엔진·감사 로그·마스킹·캐시가 전부 호출자 없이 서 있고, 설계가 실제 요청 흐름에서 맞는지 확인된 적이 없다. 청크 규칙(파일 1~3개)은 커밋의 54%만 지켰다. 계획은 16 → 128 청크로 불었고 늘린 청크 5번 중 4번이 코드를 안 썼다. 2차 점검은 API 가 붙은 뒤에 한다 | |
## 기록 규칙
