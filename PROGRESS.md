...
- **진행중 청크**: 없음
- **다음에 할 것**: **5b → 5 → 8a → 4b-1**. 1차 점검이 정한 세로 관통 순서에 5b 를 앞으로 당겼다.

  **5b 를 당긴 이유**: CSRF 가 켜져 있는데 **토큰을 받을 경로가 없다.**
  5-2 를 끝내고 실제 서버에 `curl` 로 가입을 걸어 보니 401 이다. POST 를 부를 방법이 아예 없어서
  로그인(5)도 같은 벽에 막힌다. 8a 는 GET 이라 안 걸린다.
  5b 가 할 일은 토큰 발급 경로(`CookieCsrfTokenRepository` 나 `GET /api/auth/csrf`)를 내는 것이다.
...
| 2026-08-07 | 5-2 중 드러난 것 | **실제 서버에서 가입을 못 부른다** — CSRF 가 켜져 있는데 토큰을 받을 경로가 없어서 `curl` POST 가 401 이다. MockMvc 는 `with(csrf())` 로 우회하므로 테스트만으로는 안 드러난다. 5b 를 5 앞으로 당겨서 이 벽부터 뚫는다 | |
## 기록 규칙
