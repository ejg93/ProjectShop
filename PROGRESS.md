...
- **진행중 청크**: 없음
- **다음에 할 것**: **2차 점검**. 1차 점검이 "API 가 붙은 뒤에 한다" 로 미뤄 둔 것이고, 그 조건이 찼다.
  **세로 관통이 끝났다** — 가입 → 로그인 → 권한 목록 → 감사 조회가 실서버에서 요청으로 이어진다.
  판정 엔진·감사 로그·캐시가 전부 호출자를 얻었다. 1차 점검의 최대 리스크가 해소됐다.

  **2차 점검이 볼 것**(1차와 같은 6축 + API 가 생겨서 잴 수 있게 된 것):
  테스트 비율이 내려갔나, 응답 형식이 엔드포인트마다 일관한가, `35a`(HTTP 계층 테스트)가 필요한가.
  마지막 것은 청크 `35a` 자체가 "2차 점검에서 판단한다" 로 걸려 있다.

  그 뒤에 **계정 관리 축(5h·5e·5f·5g)**, 그다음 **6·7 상품 축**.
