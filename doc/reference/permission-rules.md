...
| 스타일 | 언제 | 어디 |
|---|---|---|
| 판정 엔진 `decide` | 역할·스코프·상태에 따라 답이 갈리는 동작 — 셀러가 남의 것을, 관리자가 강제로, 감사자가 읽기만 | 나머지 전부(열다섯 클래스) |
...
**강제 지점은 `Q56` 이다** — `*Controller` 의 non-GET 입구에서 출발하는 호출 사슬에 `decide`·`allowedActions` 호출이 있거나
위 표의 자원이거나 둘 중 하나여야 한다는 것을 ArchUnit 이 잰다. 이 표가 그 규칙의 허용 목록이다.

## 알려진 구멍

지금 통과하는 것이 정상이지만 고쳐야 할 것들이다.

### 1·2. 감사자가 새 권한으로 뚫린다 · 액션 이름에 기대고 있다 — 닫는 법을 정했다(`Q59`)
...
`PermissionEvaluatorTest.KnownHole` 이 그 구멍을 「통과하는 것이 정상」으로 고정해 두고 있다.
