...
### 시드가 `V900` 대라 배포 DB 에서 새 마이그레이션이 막힌다

**빈 DB 에서는 안 보이고 배포 DB 에서만 난다.** 시드(`db/seed`·`db/seed-demo`)가 스키마와 같은 번호 줄의
`V900`~ 에 있어서, 시드를 이미 받은 DB 에 뒤에 온 `V82` 같은 스키마 마이그레이션은 「시드보다 낮은 번호」가 된다.
Flyway 는 기본으로 그것을 순서 어긋남으로 보고 **기동을 막는다** — 로그 한 줄이
`Detected resolved migration not applied to database: 82` 다. 2026-09-22~23 에 백엔드 배포가 #70~#74 동안
헬스체크 실패였고, **Railway 는 앞 판을 계속 돌려서 사이트는 떠 있었다** — 프론트만 새 판이라 새 기능을 누를 때만
실패했다. 로컬·CI·`smoke` 는 전부 빈 DB 라 한 번도 이 모양을 안 지났다.

**시드를 싣는 프로필(`demo`·`local`)이 `spring.flyway.out-of-order: true` 다**(`Q199`). 그 설정이 빠지면
`SeededUpgradeTest` 가 빨개진다. 구조를 바꿀지(시드를 번호 줄에서 떼기)는 `Q200` 이 정한다.

**머지 뒤에는 배포 상태를 본다** — 이 실패는 네 번 동안 아무 알림 없이 쌓였다.
