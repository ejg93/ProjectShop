...
축 1 에서 업계 관례는 4순위고 **근거만 대면 버린다.** 그래서 이 축의 판정은 하나다 —
**따랐든 버렸든 근거가 적혀 있나.**

**`C1` — 위 「넣었지만 안 쓰는 것」이 지나간 시점에 멈춰 있다.**

그 절이 「JPA 로 갈지 `JdbcClient` 로 갈지는 **청크 6(상품 스키마)에서** 실제 매핑이 생길 때 정한다」로
끝난다. **청크 6 은 끝났고**, 그 뒤 상품·주문·결제·환불이 전부 `JdbcClient` 로 갔다.

| | 그때 | 지금 |
|---|---|---|
| `JdbcClient` | 6곳 | 훨씬 많다(서비스·조회 전부) |
| JPA 엔티티 | 0개 | 0개 |
| 결정 | 「청크 6 에서」 | **안 적혔다** |

**실질은 이미 정해졌는데 문서가 미결이고 스타터가 남아 있다.** 다음 사람은
의존성 목록을 보고 「JPA 쓰는 프로젝트」로 읽는다. `D15` 가 화면 테스트에서 겪은 것과 같은 모양이다 —
**미착수가 아니라 정하기로 한 시점을 지난 것.**

**`C2` — 프론트가 관례 넷을 안 썼는데 근거가 한 줄도 없다.**

| 안 쓴 것 | 관례상 기본 | 우리가 대신 쓰는 것 |
|---|---|---|
| 서버 상태 관리 | `@tanstack/react-query`·`swr` | 서버 컴포넌트 + `router.refresh()` |
| 폼 | `react-hook-form` | `FormData` + `useState` |
| 날짜 | `date-fns`·`dayjs` | `toLocaleDateString` (`lib/format.ts`) |
| HTTP | `axios` | `fetch` (`lib/api.ts`) |

**넷 다 판단이 맞아 보인다** — 서버 컴포넌트가 기본이면 클라이언트 캐시 계층이 겹치고,
폼이 단순하고, 표기가 한 로케일뿐이고, `fetch` 로 충분하다.
**그런데 그 문장이 어디에도 없다.** `D23` 「안 넣은 것도 근거를 남긴다」가 정확히 이 자리다 —
근거가 없으면 다음 사람이 빠뜨린 줄 알고 채워 넣는다.

**대조되는 모범이 같은 저장소에 있다** — `eslint.config.mjs` 는 `eslint-config-next` 의 기본값을
**부분집합이라고 밝히고** 접근성 규칙을 통째로 얹은 근거를 주석에 적어 뒀다.
관례를 항목마다 재는 모양이 그것이다(`D22` 가 SQL Style Guide 를 다룬 방식과 같다).
