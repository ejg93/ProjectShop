...
| `-settlement_item.amount = commission_base_amount × commission_bp / 10000` (수수료 줄) | `check`(`V55`) |
...
**일곱째는 청크 18 이 넣었다.** 정산 항목이 금액만 들고 있으면 셀러가 정산서만으로 대사를 못 하고
주문 표를 다시 뒤져야 하는데, **그때 하는 계산이 마감 때와 같다는 보장이 없다.**
요율과 기준 금액을 정산 행에 굳혀서 등식을 한 행 안에서 검사한다.

**환급 줄에는 안 건다.** `refund_item.commission_refund` 는 원 수수료를 수량으로 나눈 값인데
마지막 수량에 절사 잔액을 몰아 주므로(아래 환불 축) **「기준액 × 요율」을 넘을 수 있다** —
등식은 물론 상한으로도 못 쓴다. 환급의 근거는 요율 재계산이 아니라 **원 수수료 금액**이고,
그 등식은 환불 축의 마지막 줄이 이미 들고 있다.
...
미인도·지연인도의 손해배상은 **액수를 법이 안 정한다** — 소비자분쟁해결기준 별표2 도
소비자기본법 시행령 별표1 도 배상액을 안 주고, 지연인도는 「구매목적을 달성했나」로 갈리는데
그것은 소비자의 사정이다(`commerce-compliance.md` R38·R39).

**그래서 이 축에 등식이 안 선다.** 여기 있는 나머지가 전부 「구한 값이 맞는지 아는 방법」인데,
배상은 구한 값이 아니라 사람이 정한 값이라 대조할 원본이 없다. 대신 **행이 갖춰야 할 것**을
제약으로 건다(`V69`) — 금액은 양수고, 사유가 비면 안 되고, 정한 사람이 반드시 있다.

**정산에 실리는 것은 `bearer = 'seller'` 뿐이다.** 우리 결제나 시스템이 멈춰서 늦은 건까지
셀러 몫에서 빼면 그 정산서가 거짓이 된다. 실릴 때 부호가 뒤집히고
(`compensation.amount` 는 양수, `settlement_item.amount` 는 음수),
`settlement_item_compensation_unique` 가 같은 판정이 두 번 실리는 것을 막는다.
