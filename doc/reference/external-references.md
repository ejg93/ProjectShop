...
**채택했다**(2026-09-14, 사용자 결정 — `event-catalog.md`). 봉투 속성 이름을 CloudEvents 1.0 그대로 쓴다.
「이벤트가 없어서 근거가 없다」던 보류 이유는 틀렸다 — `order_status_history` 같은 append-only 표가
이미 사건을 적고 있었고, 그 표들이 원천이 됐다. Zalando 에서는 **버전 규칙**(이름을 안 바꾸고 필드를 더하기만)을 빌렸다.
...
| D20 화면·문구 규약 | 접근성은 **KWCAG 2.2**(`KS X OT0003:2022`, 아래 점검 E 행)고 `jsx-a11y`·`axe-core` 가 잰다(`Q21`) — 「범위 밖」이라고 적혀 있던 것은 `Q21` 전의 말이다(2026-09-14 점검 N 이 고쳤다). 시각 디자인은 청크 `13-0` 의 스킬이 맡고, 무엇을 감추고 보이나는 우리 권한 모델에 붙은 판단이라 베낄 것이 없다 |
